...
import operator
import subprocess
import re, time, random, json, glob
import shutil
...
#class GSSubmitMuster(MayaQWidgetBaseMixin,QWidget):
#
#    data_model = {}
#    obj_list_model = None
#    renameShader = True
...
#        super(GSSubmitMuster, self).__init__(parent=parent, *args, **kwargs) 
...
class Submitter:
    ...
    def getUploadFiles(self):
        ...
        if 'GSPROJECT' in os.environ:
            ...
            # TODO make this get the config from the core.projects
            config_path = os.path.join(proj,'production','.pipeline','config')
...
#
    ...
    def save_render_file(self, sceneSuffix = '', frameSuffix = '', *args):
        # TODO need to provide the shot name here
        shot = 'temp_shot_name' #self.M.SM.checkedShot
        fullPath = cmds.file(q=1, sn=1)
        oldName = os.path.splitext(cmds.file(q=1, sn=1, shn=1))[0]
        fixedName = oldName
        if '_T_' in oldName:
    ...
    def getRenderPrefix(self, *args):
        ...
        #if 'GS_MAYA_REND_PREFIX' in os.environ:
        #    return os.environ['GS_MAYA_REND_PREFIX']
        #else:
        ...
        ver = se[0].split("_")[-1]
        return '../../../render/{0}/<RenderLayer>/{1}/<Scene>_<RenderLayer>'.format(scene,ver)
 
    def vray_prepassSetup(self, beautyLayer, *args):
        sceneName = cmds.file(q=1, sn=1)
        if '_T_' in sceneName:
            sceneName = '_T_'.join(sceneName.split('_T_')[:-1]) + '.mb'
        # TODO set a var to get the shot name
        IRPath = os.path.join(os.environ["GSPROJECT"], 'renderData', 'IRMAP', os.path.basename(sceneName))
