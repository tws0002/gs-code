...
def loadExporterUI(standalone=False):
	
	if standalone:
		wind = get_maya_window()
		widg = StudioAlembicUIWindow(parent=wind)
...
def loadImporterUI(standalone=False):
	
	if standalone == True:
		wind = get_maya_window()
		widg = StudioAlembicUIWindowImport(parent=wind)
		widg.setWindowFlags( Qt.FramelessWindowHint )
	else:
