...
var M_USER = "admin";
var M_SERVER = "dispatch.studio.gentscholar.com";
var M_PORT = "8681";
var M_MUSTEREXE = "C:\\Program Files\\Virtual Vertex\\Muster 7\\Mrtool.exe";
var M_MUSTERCONNECT = M_MUSTEREXE + ' -s ' + M_SERVER + ' -port ' + M_PORT + ' -u ' + M_USER;
var M_FOLDER = '33409';
...
function sendToFarm(compname,start,end,priority,batch,pool){
	...
	cmd = M_MUSTERCONNECT+' -b '+' -add '+'"\\\"'+M_RENDEREXE+'\\\"'+' -comp '+'\\"'+compname+'\\"\"'+' -e 1002 '+' -n '+'\"'+M_PROJECTFILE.replace(/^.*[\\\/]/, '')+'_'+compname+'\"'+' -parent '+M_FOLDER+' -pool '+pool+' -sf '+start+' -ef '+end+' -bf 1'+' -pk '+batch+' -pr '+priority+' -f '+'\"'+M_PROJECTFILE+'\"';
	//alert(cmd);	return;
	var cmdOutput = system.callSystem(cmd);
	alert(cmdOutput,M_TITLE);
	return;
