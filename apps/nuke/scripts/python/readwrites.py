...
def readwrites(blankread=False, check=False, threshold=100000, report=False):
    ...
    def _readwrites(blankread, check, threshold,  report):
        ...
        if sn == [] and blankread == True:
            ...
            for n in sn:
                ...
                args = 'file {%s} first %s last %s colorspace %s premultiplied %s raw %s' % (file, firstFrame, lastFrame, colorspace, premult, rawdata)
