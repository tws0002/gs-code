...
GSMAYA = os.path.join(GSTOOLS, 'maya')
GSMAYASCRIPT = os.path.join(GSMAYA, 'scripts')
GSMAYAMEL = os.path.join(GSMAYASCRIPT, 'mel')

ROOT_APP_PATH = {'local':'C:/Program Files', 'network':'//scholar/apps'}
APP_EXE = {'gui':'maya.exe', 'cli':'render.exe'}
...
def start(appPath):
    print "Launching Maya %s.%s from %s..." %(MAJV, MINV, LOC)
...
def main():
    ...
    envVars = {
        'PYTHONPATH'                    :   [ GSMAYASCRIPT, '%s/python' %(GSMAYASCRIPT), 'C:/code/general/scripts/python' ],
        'MAYA_SHELF_PATH'               :   [ '%s/shelves/%s' %(GSMAYA, MAJV) ],
        'MAYA_SCRIPT_PATH'              :   [ '%s;%s/Modeling;%s/Rigging;%s/Scholar;%s/Utility' %(GSMAYAMEL, GSMAYAMEL, GSMAYAMEL, GSMAYAMEL, GSMAYAMEL) ],
        'XBMLANGPATH'                   :   [ '%s/icons' %(GSMAYA) ],
        'MAYA_PRESET_PATH'              :   [ '%s/presets' %(GSMAYA) ],
        'MAYA_PLUG_IN_PATH'             :   [ '%s/plug-ins/%s' %(GSMAYA, MAJV) ],
        'TEXTURES'                      :   [ '//scholar/assets/3d/GLOBAL_TEXTURES' ],
        'AUTODESK_ADLM_THINCLIENT_ENV'  :   [ '//scholar/apps/Autodesk/adlm/AdlmThinClientCustomEnv.xml' ],
        'LM_LICENSE_FILE'               :   [ '@%s' %(LIC) ],
    }
    set_env(envVars)

    appPath = '%s/Autodesk/Maya/%s.%s/bin/%s' %(ROOT_APP_PATH[LOC], MAJV, MINV, APP_EXE[EXE])
