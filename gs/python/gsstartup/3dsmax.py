...
GSMAX = os.path.join(GSTOOLS, '3dsmax')
GSMAXSCRIPT = os.path.join(GSMAX, 'scripts')

ROOT_APP_PATH = {'local':'C:/Program Files', 'network':'//scholar/apps'}
APP_EXE = {'gui':'3dsmax.exe', 'cli':'3dsmaxcmd.exe'}
...
def install():
    netAppPath = '%s/Autodesk/3dsMax/%s.%s' %(ROOT_APP_PATH['network'], MAJV, MINV)
    locAppPath = '%s/Autodesk/3dsMax/%s.%s' %(ROOT_APP_PATH['local'], MAJV, MINV)
    ...
    if os.path.exists(netAppPath):
        if not os.path.exists(locAppPath):
            ...
            if float(appsize) < freebytes:            
                print "Installing 3dsMax %s.%s from the network. Please wait..." %(MAJV, MINV)
...
def start(appPath):
    print "Launching 3dsMax %s.%s from %s..." %(MAJV, MINV, LOC)
...
def build_render_cmd(args):
    ...
    flags = {}
    flags['-v']         = '5'
    flags['-frames']    = '%s-%s' %(r.startframe,r.endframe)

    commands = []
    for k,v in flags.items():
        commands += [k,v]
    commands.append('-batchRender')
...
def main():
    ...
    envVars = {
        'PYTHONPATH'                    :   [ GSMAXSCRIPT, '%s/python' %(GSMAXSCRIPT), 'C:/code/general/scripts/python' ],
        'AUTODESK_ADLM_THINCLIENT_ENV'  :   [ '//scholar/apps/Autodesk/adlm/AdlmThinClientCustomEnv.xml' ],
        'LM_LICENSE_FILE'               :   [ '@%s' %(LIC) ],
    }
    set_env(envVars)

    appPath = '%s/Autodesk/3dsMax/%s.%s/%s' %(ROOT_APP_PATH[LOC], MAJV, MINV, APP_EXE[EXE])
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='GS 3dsMax launcher.')
