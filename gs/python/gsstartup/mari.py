...
APP_EXE = {'gui':'Mari%sv%s.exe' %(MAJV,MINV), 'cli':'Mari%sv%s.exe' %(MAJV,MINV) }

if os.path.exists('E:'):
    MARICACHE = 'E:/Temp/Mari_Cache'
else:
    MARICACHE = 'C:/Temp/Mari_Cache'
...
def install():
    netAppPath = '%s/Foundry/Mari/%sv%s' %(ROOT_APP_PATH['network'], MAJV, MINV)
    locAppPath = '%s/Foundry/Mari/%sv%s' %(ROOT_APP_PATH['local'], MAJV, MINV)
    ...
    if os.path.exists(netAppPath):
        if not os.path.exists(locAppPath):
            ...
            if float(appsize) < freebytes:
                print "Installing Mari %sv%s from the network. Please wait..." %(MAJV, MINV)
...
def start(appPath):
    print "Launching Mari %sv%s from %s..." %(MAJV, MINV, LOC)
...
def main():
    ...
    envVars = {
        'RLM_LICENSE'       :   ['%s' %(LIC)],
        'MARI_CACHE'        :   [MARICACHE],
        'MARI_SCRIPT_PATH'  :   [os.path.join(GSTOOLS, 'mari', 'scripts')]
    }
    set_env(envVars)

    APP_EXE = {'gui':'Mari%sv%s.exe' %(MAJV,MINV), 'cli':'Mari%sv%s.exe' %(MAJV,MINV) }
    appPath = '%s/Foundry/Mari/%sv%s/Bundle/bin/%s' %(ROOT_APP_PATH[LOC], MAJV, MINV, APP_EXE[EXE])
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='GS Mari launcher.')
    parser.add_argument('file', nargs='?', help='File to open.')
    parser.add_argument('-p', '--package', nargs=1, choices=['Marix', 'Mariassist', 'studio'], help='Choose Mari package (options are Marix, Mariassist, studio).')
